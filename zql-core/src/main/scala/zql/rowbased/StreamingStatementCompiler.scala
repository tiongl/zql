...
class StreamingStatementCompiler[ROW](val table: StreamingTable[ROW]) extends StatementCompiler[Table] with AccessorCompiler {
      ...
      first("Filter the data") {
        ...
        filteredData =>
          ...
          val groupedProcessData = if (stmt.groupBy != null) {
            ...
            val selected = filteredData.map(d => (1, selectFunc(d)))
            val grouped = selected.groupBy(t => new Row(Array(t._1)), t => t._2, groupByIndices)
            grouped.map(_.normalize)
          } else {
